...
Deno.test("Commit", async (t) => {
	await t.step("create", async () => {
		const commit1 = Commit.new({ tree: "tree_hash", commiter: "commiter", date: new Date("2025-10-01T10:19:00.000Z"), message: "message" });
	...
	await t.step("fromArrayBuffer", async () => {
		const commit1 = Commit.new({ tree: "tree_hash", commiter: "commiter", date: new Date(), message: "message" });
