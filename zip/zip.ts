...
import { File, FileExt, SeekFrom } from "@pxlr/vfs";
...
export class Zip {
	...
	async open(
		...
		try {
			...
			try {
				...
				await this.#fileExt.seek(offsetEOCDR, SeekFrom.End);
				await this.#fileExt.read(this.#eocdr.arrayBuffer);
				...
				if (
					...
					const offsetEOCDL = await this.#fileExt.seek(offsetEOCDR - 20, SeekFrom.End);
					await this.#fileExt.read(this.#zeocdl.arrayBuffer);
					...
					await this.#fileExt.seek(this.#zeocdl.offsetToCentralDirectory, SeekFrom.Start);
					await this.#fileExt.read(this.#zeocdr.arrayBuffer);
				...
				try {
					...
					this.#offsetToPxlrHeader = await this.#fileExt.seek(
						this.#offsetToCentralDirectory - this.#pxh.arrayBuffer.byteLength,
						SeekFrom.Start,
					);
					await this.#fileExt.read(this.#pxh.arrayBuffer);
				...
				await this.#fileExt.seek(offsetToCentralDirectory, SeekFrom.Start);
				for (
					let i = 0, j = 0;
					...
					await this.#fileExt.read(cdfhFixed.arrayBuffer);
					...
					await this.#fileExt.read(variableData);
	...
	async #findEndOfCentralDirectoryRecord(offset = -22): Promise<number> {
		...
		for (; offset >= -0xFFFF;) {
			try {
				await this.#fileExt.seek(offset, SeekFrom.End);
				const byteRead = await this.#fileExt.read(buffer);
				if (byteRead === null) {
					break;
				}
	...
	*iterCentralDirectoryFileHeader(): IterableIterator<
	...
	async #getLocalFileHeader(
		...
		await this.#fileExt.seek(localFileOffset, SeekFrom.Start);
		abortSignal?.throwIfAborted();
		const lfhFixed = new LocalFileHeader(30);
		await this.#fileExt.read(lfhFixed.arrayBuffer);
	...
	async get(fileName: string, abortSignal?: AbortSignal): Promise<Uint8Array<ArrayBuffer>> {
		const releaseLock = await this.#lock.acquire();
		try {
			if (!this.#file || !this.#fileExt) {
				throw new ZipClosedError();
			...
			await this.#fileExt.read(compressedData);
			abortSignal?.throwIfAborted();
	...
	async getStream(
		...
		try {
			...
			const controllerTransform = new TransformStream<Uint8Array<ArrayBuffer>>({
				flush: () => {
					releaseLock();
				},
			});
			...
			if (lfh.compressionMethod === 0) {
				const readableStream = await this.#fileExt.read(compressedSize);
	...
	async put(
		...
		try {
			...
			let byteWritten = 0;
			byteWritten += await this.#fileExt.write(lfh.arrayBuffer);
			abortSignal?.throwIfAborted();
			byteWritten += await this.#fileExt.write(compressedData);
			abortSignal?.throwIfAborted();
			cdfh.isUpdated = true;
	...
	async putStream(
		...
		try {
			...
			const contentWritableStream = await this.#fileExt.write();
			...
			return new WritableStream({
				...
				close: async () => {
					...
					byteWritten += await this.#fileExt!.write(z64dd.arrayBuffer);
	...
	async #writeCentralDirectory(abortSignal?: AbortSignal) {
		if (!this.#file || !this.#fileExt || !this.#eocdr || !this.#isCentralDirectoryDirty) {
			throw new ZipClosedError();
		}
		let byteWritten = 0;
		...
		if (
			...
			if (this.#writeCursor >= offsetToPaddingEnd) {
				if (this.#options.centralDirectoryPaddingSize) {
					await this.#fileExt.seek(this.#writeCursor, SeekFrom.Start);
					this.#writeCursor += await this.#fileExt.write(
				...
				await this.#fileExt.seek(this.#offsetToPxlrHeader, SeekFrom.Start);
				this.#pxh = new PxlrHeader();
				this.#pxh.sizeOfPadding = currentPaddingSize;
				await this.#fileExt.write(this.#pxh.arrayBuffer);
			}
		}
			...
			if (cdfh.centralDirectoryFileHeaderOffset >= this.#writeCursor) {
				if (
					...
					if (firstNonCorruptedCDFH) {
						...
						await this.#fileExt.seek(offset, SeekFrom.Start);
					}
					if (cdfh.isUpdated) {
						byteWritten += await this.#fileExt.write(cdfh.arrayBuffer);
					} else {
						await this.#fileExt.seek(
		...
		const endOfCentralDirectory = await this.#fileExt.seek(0, SeekFrom.Current);
		...
		if (
			...
			await this.#fileExt.write(zeocdr.arrayBuffer);
			...
			await this.#fileExt.write(zeocdl.arrayBuffer);
			byteWritten += zeocdl.arrayBuffer.byteLength;
			abortSignal?.throwIfAborted();
			this.#zeocdl = zeocdl;
		}
		// Write EndOfCentralDirectoryRecord
		await this.#fileExt.write(eocdr.arrayBuffer);
		...
		const endOfFile = await this.#fileExt.seek(0, SeekFrom.Current);
		await this.#fileExt.truncate(endOfFile);
...
export class CompressionMethodNotSupportedError extends Error {
	public override name = "CompressionMethodNotSupportedError";
